...
var init_web_loader = __esm({
  "loaders/embedjs-loader-web/src/web-loader.ts"() {
    ...
    WebLoader = class extends BaseLoader {
      debug = createDebugMessages10("embedjs:loader:WebLoader");
      urlOrContent;
      isUrl;
      constructor({
        urlOrContent,
...
// databases/embedjs-pinecone/src/pinecone-db.ts
import { Pinecone } from "@pinecone-database/pinecone";
import createDebugMessages11 from "debug";
var PineconeDb = class _PineconeDb {
  debug = createDebugMessages11("embedjs:vector:PineconeDb");
  static PINECONE_INSERT_CHUNK_SIZE = 200;
  ...
  async insertChunks(chunks) {
    ...
    for (let i = 0; i < chunks.length; i += _PineconeDb.PINECONE_INSERT_CHUNK_SIZE) {
      const chunkBatch = chunks.slice(i, i + _PineconeDb.PINECONE_INSERT_CHUNK_SIZE);
...
// examples/pinecone/src/main.ts
var llmApplication = await new RAGApplicationBuilder().setEmbeddingModel(new OpenAiEmbeddings()).setVectorDatabase(
  new PineconeDb({
    projectName: "test",
    namespace: "dev",
    indexSpec: {
