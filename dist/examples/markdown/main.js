...
// core/embedjs/src/loaders/local-path-loader.ts
init_src();
var LocalPathLoader = class extends BaseLoader {
  debug = createDebugMessages14("embedjs:loader:LocalPathLoader");
  path;
  constructor({ path: path2 }) {
    super(`LocalPathLoader_${md513(path2)}`, { path: path2 });
    this.path = path2;
  }
  async *getUnfilteredChunks() {
  ...
  async *recursivelyAddPath(currentPath) {
    const isDir = fs5.lstatSync(currentPath).isDirectory();
    this.debug(`Processing path '${currentPath}'. It is a ${isDir ? "Directory!" : "file..."}`);
    if (!isDir) {
      const stream = fs5.createReadStream(currentPath);
      let { mime: mime2 } = await getMimeType2(stream);
      stream.destroy();
      this.debug(`File '${this.path}' has mime type '${mime2}'`);
      ...
      try {
        const loader = await createLoaderFromMimeType(currentPath, mime2);
      ...
      const files = fs5.readdirSync(currentPath);
