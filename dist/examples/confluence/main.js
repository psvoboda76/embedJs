...
var init_web_loader = __esm({
  "loaders/embedjs-loader-web/src/web-loader.ts"() {
    ...
    WebLoader = class extends BaseLoader {
      debug = createDebugMessages12("embedjs:loader:WebLoader");
      urlOrContent;
      isUrl;
      constructor({
        urlOrContent,
        ...
        super(`WebLoader_${md56(urlOrContent)}`, { urlOrContent }, chunkSize ?? 2e3, chunkOverlap ?? 0);
...
var LibSqlDb = class {
  debug = createDebugMessages10("embedjs:vector:LibSqlDb");
  tableName;
  client;
  constructor({ path: path2, tableName }) {
  ...
  async insertChunks(chunks) {
    ...
    this.debug(`Executing batch - ${truncateCenterString(JSON.stringify(batch), 1e3)}`);
...
var LibSqlStore = class {
  debug = createDebugMessages11("embedjs:store:LibSqlStore");
...
var ConfluenceLoader = class extends BaseLoader {
  debug = createDebugMessages13("embedjs:loader:ConfluenceLoader");
  confluence;
  confluenceBaseUrl;
  spaceName;
  lastUpdatedFilter;
  constructor({
    ...
    super(`ConfluenceLoader_${md57(spaceName)}`, { spaceName }, chunkSize ?? 2e3, chunkOverlap ?? 200);
  ...
  async *processPage(pageId, title) {
    ...
    try {
      ...
      if (content.children) {
        for (const { id, title: title2 } of content.children.page.results) {
          try {
            for await (const result of this.processPage(id, title2)) {
              yield result;
            }
          } catch (e) {
            this.debug(`Error! Could not process page child '${title2}'`, pageId, e);
            return;
          }
        }
      }
...
// examples/confluence/src/main.ts
var databasePath = path.resolve("./examples/confluence/data.db");
var llmApplication = await new RAGApplicationBuilder().setStore(new LibSqlStore({ path: databasePath })).setVectorDatabase(new LibSqlDb({ path: databasePath })).setModel(new OpenAi({ modelName: "gpt-4o" })).setEmbeddingModel(new OpenAiEmbeddings()).build();
